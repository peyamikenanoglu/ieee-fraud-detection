...
################################################
# Load Baseline v2 Dataset
################################################

df = pd.read_csv(PROCESSED_DATA_DIR / "baseline_v2_train.csv")

target_col = "isFraud"
id_col = "TransactionID"

print("##################### Loaded Baseline v2 Data #####################")
print(df.shape)
...
print("##################### LightGBM v2 Best Threshold by F1 #####################")
print(best_f1_row)
...
lgbm_v2_metrics = pd.DataFrame([lgbm_v2_results])
lgbm_v2_metrics.to_csv(
    METRICS_DIR / "lgbm_baseline_v2_metrics.csv",
    index=False
)

threshold_results_df.to_csv(
    METRICS_DIR / "lgbm_baseline_v2_fine_threshold_results.csv",
    index=False
)
...
plt.ylabel("Score")
plt.title("LightGBM Baseline v2 - Threshold Analysis")
plt.legend()
plt.tight_layout()
plt.savefig(FIGURES_DIR / "lgbm_baseline_v2_threshold_analysis.png", dpi=300)
plt.show()

print("##################### Saved Baseline v2 Outputs #####################")
print(METRICS_DIR / "lgbm_baseline_v2_metrics.csv")
print(METRICS_DIR / "lgbm_baseline_v2_fine_threshold_results.csv")
print(FIGURES_DIR / "lgbm_baseline_v2_threshold_analysis.png")


################################################
# Compare LightGBM Baseline v1 vs v2
################################################

lgbm_v1_v2_comparison = pd.DataFrame([
    {
...
print("##################### LightGBM v1 vs v2 Comparison #####################")
print(lgbm_v1_v2_comparison)

comparison_output_path = METRICS_DIR / "lgbm_v1_v2_comparison.csv"
lgbm_v1_v2_comparison.to_csv(comparison_output_path, index=False)

print("##################### Saved LightGBM v1 vs v2 Comparison Table #####################")
print(comparison_output_path)


################################################
# Plot LightGBM v1 vs v2 Comparison
################################################

plot_metrics = ["roc_auc", "pr_auc", "best_f1", "logloss"]

plot_df = lgbm_v1_v2_comparison.set_index("feature_set")[plot_metrics]

plt.figure(figsize=(10, 6))
plot_df.plot(kind="bar")
plt.title("LightGBM Performance Comparison: Baseline v1 vs v2")
plt.ylabel("Score")
plt.xticks(rotation=0)
plt.tight_layout()

comparison_figure_output_path = FIGURES_DIR / "lgbm_v1_v2_comparison.png"
plt.savefig(comparison_figure_output_path, dpi=300)
plt.show()

print("##################### Saved LightGBM v1 vs v2 Comparison Figure #####################")
print(comparison_figure_output_path)


################################################
# Feature Importance - LightGBM Baseline v2
################################################
...
feature_importance_output_path = METRICS_DIR / "lgbm_baseline_v2_feature_importance.csv"
...
plt.title("LightGBM Baseline v2 - Top 30 Feature Importances")
plt.tight_layout()

feature_importance_figure_path = FIGURES_DIR / "lgbm_baseline_v2_feature_importance_top30.png"
...
################################################
# Email Feature Importance Check
################################################
...
email_feature_importance_df = feature_importance_df[
    feature_importance_df["feature"].str.contains("|".join(email_keywords), case=False, regex=True)
].copy()

print("##################### Email Feature Importances #####################")
print(email_feature_importance_df)

email_feature_importance_output_path = METRICS_DIR / "lgbm_baseline_v2_email_feature_importance.csv"
email_feature_importance_df.to_csv(email_feature_importance_output_path, index=False)

print("##################### Saved Email Feature Importance Table #####################")
print(email_feature_importance_output_path)